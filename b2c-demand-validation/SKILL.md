...
| When in the year is interest highest? | `seasonality` |
| Is the audience on mobile or desktop? | `platform_mix` |
...
- `seasonality`: in which months is interest highest and lowest? A monthly index Jan..Dec (1.0 = an average month), averaged over the years of the period with each year normalised by its own mean; peak months (index >= 1.15), low months (<= 0.85) and the amplitude max / min (below 1.3 no real seasonality, 1.3 .. 2 moderate, above 2 strong). Needs at least 24 full calendar months; 36 recommended. Partial months at either end are dropped, and years are counted back from the last full month. If `seasonal_consistency` warns, the pattern changes from year to year: say the timing is not dependable.
- `platform_mix`: is the audience mostly on mobile or on desktop? Each platform's share of the views (`desktop`, `mobile_web`, `mobile_app`) and `mobile_total` (web + app): above 0.65 mobile-first, 0.35 .. 0.65 mixed, below 0.35 desktop-first. Needs a period of at least 28 days; 3-12 months recommended. It fetches every article three times, once per platform. A `fetch_errors` warning about the mobile app is common (many articles have little app data): the shares then come from desktop and mobile web, so say the app share may be understated. Wikipedia as a whole is mostly read on mobile, so a mobile-first result means more when you compare it with other subjects.
